...
double cal_theta(const double x) {
    return 0.5 * M_PI * (erf(B * (x - C)) - erf(B * (x + C)));
}

double cal_der_theta(const double x) {
    return sqrt(M_PI) * B * (exp(-B * B * (x - C) * (x - C)) - exp(-B * B * (x + C) * (x + C)));
}
...
void sys(const state_t& state, state_t& state_dot, const double /* t */) {
    ...
    if (s == 0) {
        // vx <-> v0x
        double v0x = vx;
        double v1x = sqrt(v0x * v0x + 2 * mass * eva[0] - 2 * mass * eva[1]);
        ...
        // vx <-> v1x
        double v1x = vx;
        double v0x = sqrt(v1x * v1x + 2 * mass * eva[1] - 2 * mass * eva[0]);
